...
{
if(temp->start > temp->next->start) //1st value with 2nd value
{
val = temp->start; //SWAPPING START
temp->start = temp->next->start;
temp->next->start = val;
val = temp->end; //SWAPPING END
temp->end = temp->next->end;
temp->next->end = val;
val = temp->min; //SWAPPING MIN
temp->min = temp->next->min;
temp->next->min = val;
val = temp->max; //SWAPPING MAX
